...
const WordDisplay: React.FC<WordDisplayProps> = ({ word, characterMap, isDarkMode = false }) => {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto space-y-6">
      {/* Main Word Display */}
      <Card className={`p-8 text-center border-0 shadow-lg ${isDarkMode
        ? 'bg-gradient-to-br from-gray-800 to-gray-700'
        : 'bg-gradient-to-br from-blue-50 to-indigo-50'
        }`}>
        <div className="space-y-4">
          <div className={`text-6xl md:text-8xl font-bold mb-4 tracking-wide ${isDarkMode ? 'text-white' : 'text-gray-800'
            }`}>
            {word.native}
          </div>
          <div className={`text-2xl md:text-3xl font-medium ${isDarkMode ? 'text-blue-400' : 'text-blue-600'
            }`}>
            {word.romanized}
          </div>
          <div className={`text-xl md:text-2xl font-light ${isDarkMode ? 'text-gray-300' : 'text-gray-600'
            }`}>
            {word.translation}
          </div>
        </div>
      ...
      <Card className={`p-6 border-0 shadow-lg ${isDarkMode ? 'bg-gray-800' : 'bg-white'
        ...
        <h3 className={`text-xl font-semibold mb-6 text-center ${isDarkMode ? 'text-white' : 'text-gray-800'
          }`}>
          Character Breakdown
        </h3>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
          {breakdown.map((item, index) => (
            <div
              key={index}
              className={`p-4 rounded-lg border hover:shadow-md ${isDarkMode
            ...
            >
              <div className="text-center space-y-2">
                ...
                <div className={`text-lg font-medium ${isDarkMode ? 'text-purple-400' : 'text-purple-600'
                  }`}>
                  {item.sound || '(silent)'}
                </div>
